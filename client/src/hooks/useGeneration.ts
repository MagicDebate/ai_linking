...
export function useGeneration() {
  ...
  const startGenerationMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success('Генерация ссылок запущена!');
      console.log('✅ Generation started:', data);
    },
    onError: (error: Error) => {
      toast.error(`Ошибка запуска генерации: ${error.message}`);
      console.error('❌ Generation start error:', error);
    },
  });

  // Get generation progress
  const useGenerationProgress = (runId: string | null) => {
    return useQuery({
  ...
  // Get draft results
  const useDraftResults = (runId: string | null, filters?: { type?: string; status?: string; limit?: number; offset?: number }) => {
    return useQuery({
